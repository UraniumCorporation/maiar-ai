...
export class Physics extends System {
  ...
  async init() {
    ...
    simulationEventCallback.onContact = (pairHeader, pairs, count) => {
      ...
      for (let i = 0; i < count; i++) {
        const pair = PHYSX.NativeArrayHelpers.prototype.getContactPairAt(
          pairs,
          i
        );
    ...
    simulationEventCallback.onTrigger = (pairs, count) => {
      ...
      for (let i = 0; i < count; i++) {
        const pair = PHYSX.NativeArrayHelpers.prototype.getTriggerPairAt(
          pairs,
          i
        );
        ...
        if (
          pair.flags.isSet(
            PHYSX.PxTriggerPairFlagEnum.eREMOVED_SHAPE_TRIGGER
          ) ||
          pair.flags.isSet(PHYSX.PxTriggerPairFlagEnum.eREMOVED_SHAPE_OTHER)
        ) {
          continue;
        }
        const triggerHandle = this.handles.get(
          pair.triggerShape.getActor().ptr
        );
  ...
  postFixedUpdate(delta) {
    ...
    this.processTriggerCallbacks();
    const activeActors =
      PHYSX.SupportFunctions.prototype.PxScene_getActiveActors(this.scene);
