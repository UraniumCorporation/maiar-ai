...
export class RigidBody extends Node {
  ...
  mount() {
    ...
    if (this._type === "static") {
      this.actor = this.ctx.world.physics.physics.createRigidStatic(
        this.transform
      );
    } else if (this._type === "kinematic") {
      this.actor = this.ctx.world.physics.physics.createRigidDynamic(
        this.transform
      );
      this.actor.setRigidBodyFlag(PHYSX.PxRigidBodyFlagEnum.eKINEMATIC, true);
      // this.actor.setMass(this.mass)
      PHYSX.PxRigidBodyExt.prototype.setMassAndUpdateInertia(
        this.actor,
        this._mass
      );
  ...
  getVelocityAtPos(pos, vec3) {
    ...
    if (!this.actor) return vec3.set(0, 0, 0);
    return vec3.copy(
      PHYSX.PxRigidBodyExt.prototype.getVelocityAtPos(
        this.actor,
        pos.toPxVec3()
      )
    );
  ...
  getLocalVelocityAtLocalPos(pos, vec3) {
    ...
    if (!this.actor) return vec3.set(0, 0, 0);
    return vec3.copy(
      PHYSX.PxRigidBodyExt.prototype.getLocalVelocityAtLocalPos(
        this.actor,
        pos.toPxVec3()
      )
    );
