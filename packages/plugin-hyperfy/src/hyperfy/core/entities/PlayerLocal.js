...
export class PlayerLocal extends Entity {
  ...
  toggleFlying() {
    const canFly =
      this.world.settings.public || hasRole(this.data.roles, "admin");
    if (!canFly) return;
  ...
  fixedUpdate(delta) {
    ...
    if (anchor && !this.capsuleDisabled) {
      this.capsule.setActorFlag(
        PHYSX.PxActorFlagEnum.eDISABLE_SIMULATION,
        true
      );
    ...
    if (anchor) {
      ...
      let sweepHit;
      {
        ...
        const hitMask = Layers.environment.group | Layers.prop.group;
        sweepHit = this.world.physics.sweep(
          geometry,
          origin,
          direction,
          maxDistance,
      ...
      let perpComponent = v2
        .copy(this.groundNormal)
        .multiplyScalar(velocity.dot(this.groundNormal));
      ...
      if (this.grounded && !this.jumping) {
        ...
        const projectedVector = v2
          .copy(this.groundNormal)
          .multiplyScalar(projectedLength);
  ...
  update(delta) {
    ...
    if (isXR) {
      ...
      this.cam.rotation.x +=
        -this.control.pointer.delta.y * POINTER_LOOK_SPEED * delta;
      this.cam.rotation.y +=
        -this.control.pointer.delta.x * POINTER_LOOK_SPEED * delta;
    ...
    this.jumpDown = isXR
      ? this.control.xrRightBtn1.down
      : this.control.space.down || this.control.touchA.down;
    if (
      isXR
        ? this.control.xrRightBtn1.pressed
        : this.control.space.pressed || this.control.touchA.pressed
    ) {
