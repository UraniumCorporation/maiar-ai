...
export class ServerNetwork extends System {
  ...
  save = async () => {
    ...
    // log
    const didSave =
      counts.upsertedBlueprints > 0 ||
      counts.upsertedApps > 0 ||
      counts.deletedApps > 0;
    if (didSave) {
      console.log(
  ...
  onBlueprintAdded = (socket, blueprint) => {
    if (!this.isBuilder(socket.player)) {
      return console.error(
        "player attempted to add blueprint without builder permission"
      );
    }
  ...
  onBlueprintModified = (socket, data) => {
    if (!this.isBuilder(socket.player)) {
      return console.error(
        "player attempted to modify blueprint without builder permission"
      );
    }
  ...
  onEntityAdded = (socket, data) => {
    if (!this.isBuilder(socket.player)) {
      return console.error(
        "player attempted to add entity without builder permission"
      );
    }
  ...
  onEntityRemoved = (socket, id) => {
    if (!this.isBuilder(socket.player))
      return console.error(
        "player attempted to remove entity without builder permission"
      );
    const entity = this.world.entities.get(id);
  ...
  onSettingsModified = (socket, data) => {
    if (!this.isBuilder(socket.player))
      return console.error(
        "player attempted to modify settings without builder permission"
      );
  ...
  onSpawnModified = async (socket, op) => {
    if (!this.isBuilder(socket.player)) {
      return console.error(
        "player attempted to modify spawn without builder permission"
      );
    }
    const player = socket.player;
    if (op === "set") {
