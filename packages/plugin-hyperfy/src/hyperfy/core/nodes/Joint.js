...
export class Joint extends Node {
  constructor(data = {}) {
    ...
    this.breakForce = isNumber(data.breakForce)
      ? data.breakForce
      : defaults.breakForce;
    this.breakTorque = isNumber(data.breakTorque)
      ? data.breakTorque
      : defaults.breakTorque;
    ...
    this.limitStiffness = isNumber(data.limitStiffness)
      ? data.limitStiffness
      : defaults.limitStiffness;
    this.limitDamping = isNumber(data.limitDamping)
      ? data.limitDamping
      : defaults.limitDamping;
  ...
  mount() {
    ...
    if (this.type === "fixed") {
      ...
      // make joint
      this.joint = new PHYSX.FixedJointCreate(
    ...
    if (this.type === "socket") {
      ...
      // make joint
      this.joint = new PHYSX.SphericalJointCreate(
        this.ctx.world.physics.physics,
        actor0,
        frame0,
        actor1,
      ...
      if (isNumber(this.limitY) && isNumber(this.limitZ)) {
        ...
        this.joint.setSphericalJointFlag(
          PHYSX.PxSphericalJointFlagEnum.eLIMIT_ENABLED,
          true
        );
        PHYSX.destroy(cone);
    ...
    if (this.type === "hinge") {
      ...
      _q2.copy(this.quaternion1).multiply(alignRotation).toPxTransform(frame1);
      // make joint
      this.joint = new PHYSX.RevoluteJointCreate(
        this.ctx.world.physics.physics,
        actor0,
        frame0,
        actor1,
      ...
      if (isNumber(this.limitMin) && isNumber(this.limitMax)) {
        ...
        const limit = new PHYSX.PxJointAngularLimitPair(
          this.limitMin * DEG2RAD,
          this.limitMax * DEG2RAD,
          spring
        );
        this.joint.setLimit(limit);
        this.joint.setRevoluteJointFlag(
          PHYSX.PxRevoluteJointFlagEnum.eLIMIT_ENABLED,
          true
        );
        PHYSX.destroy(limit);
        if (spring) PHYSX.destroy(spring);
    ...
    if (this.type === "distance") {
      ...
      // _q2.copy(this.quaternion1).multiply(alignRotation).toPxTransform(frame1)
      // make joint
      this.joint = new PHYSX.DistanceJointCreate(
        this.ctx.world.physics.physics,
        actor0,
        frame0,
        actor1,
      ...
      this.joint.setDistanceJointFlag(
        PHYSX.PxDistanceJointFlagEnum.eMIN_DISTANCE_ENABLED,
        true
      );
      this.joint.setDistanceJointFlag(
        PHYSX.PxDistanceJointFlagEnum.eMAX_DISTANCE_ENABLED,
        true
      );
      if (isNumber(this.limitStiffness) && isNumber(this.limitDamping)) {
        ...
        this.joint.setDistanceJointFlag(
          PHYSX.PxDistanceJointFlagEnum.eSPRING_ENABLED,
          true
        );
      }
    }
    ...
    if (this.collide) {
      this.joint.setConstraintFlag(
        PHYSX.PxConstraintFlagEnum.eCOLLISION_ENABLED,
        true
      );
    }
