  ...
  return {
    ...
    teleport(position, rotationY) {
      if (player.data.owner === world.network.id) {
        ...
        world.network.enqueue("onPlayerTeleport", {
        ...
        world.network.sendTo(player.data.owner, "playerTeleport", {
          position: position.toArray(),
          rotationY
        });
      }
    ...
    setSessionAvatar(url) {
      ...
      if (player.data.owner === world.network.id) {
        ...
        // if we're a client we need to notify server
        world.network.send("playerSessionAvatar", {
          networkId: player.data.owner,
          avatar
        });
      } else {
        // if we're the server we need to notify the player
        world.network.sendTo(player.data.owner, "playerSessionAvatar", {
          avatar
        });
      }
    },
