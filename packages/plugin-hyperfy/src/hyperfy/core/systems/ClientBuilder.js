...
export class ClientBuilder extends System {
  ...
  canBuild() {
    return (
      ...
      hasRole(this.world.entities.player?.data.roles, "admin")
    );
  }
  ...
  updateActions() {
    ...
    if (this.enabled && !this.selected) {
      ...
      actions.push({
        type: "custom",
        btn: "1234",
        label: "Grab / Translate / Rotate / Scale"
      });
    ...
    if (
      ...
      actions.push({
        type: "keyT",
        label: this.localSpace ? "World Space" : "Local Space"
      });
      actions.push({ type: "keyX", label: "Destroy" });
      actions.push({ type: "controlLeft", label: "No Snap (Hold)" });
      actions.push({ type: "space", label: "Jump / Fly (Double-Tap)" });
  ...
  update(delta) {
    ...
    // inspect out of pointer-lock
      ...
      this.control.mouseRight.pressed
    ) {
    ...
    if (
      this.selected &&
      this.mode === "rotate" &&
      this.control.controlLeft.pressed
    ) {
    ...
    if (
      this.selected &&
      this.mode === "rotate" &&
      this.control.controlLeft.released
    ) {
    ...
    if (this.selected && this.mode === "grab") {
      ...
      const camDir = v1
        .copy(FORWARD)
        .applyQuaternion(this.world.rig.quaternion);
      ...
      if (hit && hitDistance < this.target.limit) {
        ...
        this.target.position
          .copy(camPos)
          .add(camDir.multiplyScalar(this.target.limit));
      }
      // if holding F/C then push or pull
      let project = this.control.keyF.down
        ? 1
        : this.control.keyC.down
          ? -1
          : null;
      if (project) {
      ...
      if (!this.control.controlLeft.down) {
        ...
        const snappedDegrees =
          Math.round(degrees / SNAP_DEGREES) * SNAP_DEGREES;
  ...
  select(app) {
    ...
    if (this.selected && this.selected !== app) {
      if (
        !this.selected.dead &&
        this.selected.data.mover === this.world.network.id
      ) {
        const app = this.selected;
        app.data.mover = null;
    ...
    if (e.dataTransfer.items && e.dataTransfer.items.length > 0) {
      ...
      if (
        ...
        if (url.startsWith("http")) {
          ...
          file = new File([blob], new URL(url).pathname.split("/").pop(), {
